...
3. **Responsibility Assignment** — who is expected/allowed to act on a specific Responsibility (Section 5.2's assignee set) — a person can be assigned without broad visibility being separately granted; correct behavior requires that assignment implies at minimum the visibility needed to act on that specific Responsibility, without implying visibility into the rest of the Object beyond that.
