...
| **TD-015** | **Application-Layer CQRS Façade Use Cases** | Medium | Phase 3 (Refactoring) | `Tracked` | Temporary service façade Use Cases (`CreateObjectFacadeUseCase`, `Reminder` façades, `Space` façades, `Collection` façades, `Workspace` façades, `User` façades) delegate internally to legacy NestJS services. These will be converted to pure Domain Repositories & Aggregates once `PrismaObjectRepository`, `PrismaReminderRepository`, etc. are fully constructed. | 2 Sprints |
| **TD-016** | **Structured Exception Code Mapping in Transport Adapters** | Low | Phase 3 (Architecture) | `Tracked` | Controller transport adapters currently map `Result` error messages using string matching (`.includes('not found')`). Standardizing to explicit `ApplicationException.code` switch mapping or global NestJS exception filter will improve resilience. | 1 Sprint |
| **TD-017** | **Auth Bounded Context CQRS Migration** | Medium | Phase 3 (Security) | `Tracked` | `AuthController` currently delegates directly to `AuthService`. Refactoring to dedicated Use Cases (`RegisterUserUseCase`, `AuthenticateUserUseCase`, `RefreshTokenUseCase`) deferred to Phase 3 core security overhaul. | 1 Sprint |
| **TD-018** | **Repository Aggregate Rehydration Synthetic Fields** | Low | Phase 3 (Persistence) | `Tracked` | `PrismaNotificationRepository` & `PrismaTimelineRepository` rehydrate missing persistence fields using explicit defaults (e.g. `readAt`, `failureReason`, `attempts`) until dedicated columns exist in Prisma schema migration v2. | 1 Sprint |
