...
3. Maps the code to an HTTP status code via an exhaustive `switch` statement.
4. Returns a standardized error body: `{ error: { code, message, details? } }`.
...
| `DOMAIN_VALIDATION_ERROR`, `REVISION_CONFLICT`, `EMAIL_ALREADY_REGISTERED`, `USERNAME_TAKEN`, `OAUTH_ACCOUNT_RESOLUTION_FAILED`, `PERSONAL_WORKSPACE_DELETION_FORBIDDEN`, `TARGET_NOT_WORKSPACE_MEMBER`, `RECURRENCE_RULE_INVALID`, `REMINDER_ONLY_ACTIVE_CAN_SNOOZE` | 400 Bad Request |
| Any unmapped code (default) | 500 Internal Server Error |
...
- **Type safety**: Adding a new `ErrorCode` will produce a TypeScript exhaustiveness warning if the filter switch is not updated (when `noImplicitReturns` is enabled).
- **Testable in isolation**: The filter is a plain class with a single static method; the mapping table is unit-testable without a running NestJS application.
