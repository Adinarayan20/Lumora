...
| 05 | `05_ROADMAP_IMPLEMENTATION_CONTRACT.md` | Pending | V1/NEXT/LATER/FUTURE sequencing at implementation granularity, acceptance criteria, repository migration plan (KEEP/REMOVE/REBUILD) | Engineering constitution rules (owned by `.agents/AGENTS.md`), redefinition of product or domain decisions (references 01/02) |
| — | `.agents/AGENTS.md` | **Active** | Durable engineering constitution: engineering rules, coding standards, security engineering rules, maintainability rules, testing discipline, implementation boundaries | Product intent (01), domain definitions (02), system architecture (03), security/data policy (04), implementation sequencing (05) |

**Engineering Constitution:** `.agents/AGENTS.md` is the durable engineering constitution. It governs coding standards, engineering principles, quality rules, and implementation discipline across the repository. Documents `00`–`04` are the authoritative product, domain, system architecture, and data/security contracts. Document `05` owns implementation sequencing, acceptance criteria, and repository migration contracts; it must not duplicate or replace `.agents/AGENTS.md`.

**Stated plainly:** `01` is product authority — why Lumora exists and what a person experiences. `02` is domain authority — exactly what every term means, and the rules that make it correct. `.agents/AGENTS.md` is engineering constitution authority — the rules for how code is engineered, tested, and maintained. None substitutes for the other, and all are binding within their respective owned scopes on `03` and subsequent implementation.
