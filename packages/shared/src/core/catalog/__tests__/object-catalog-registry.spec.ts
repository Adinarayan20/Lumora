...
describe("ObjectCatalogRegistry", () => {
  it("should retrieve metadata for built-in object types with stable i18n and icon keys", () => {
    ...
    expect(taskMeta?.supportedExtensions).toContain(
  ...
  it("should verify supported extension bindings", () => {
    expect(
      ObjectCatalogRegistry.supportsExtension(
        ObjectTypeKey.TASK,
        BehaviorExtensionKey.REMINDER,
      ),
    ).toBe(true);
  ...
  it("should assert valid type keys and throw DomainValidationException for unregistered keys", () => {
    expect(() =>
      ObjectCatalogRegistry.assertValidTypeKey(ObjectTypeKey.NOTE),
    ).not.toThrow();
    expect(() =>
      ObjectCatalogRegistry.assertValidTypeKey("INVALID_TYPE_KEY"),
