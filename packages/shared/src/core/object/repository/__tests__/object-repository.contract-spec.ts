...
export function runObjectRepositoryContractTests(
  ...
  describe('Universal Object Repository Behavioral Contract Suite', () => {
    ...
    it('rejects creation of duplicate object ID with ObjectAlreadyExistsException', async () => {
      ...
      await expect(repo.create(sampleObject)).rejects.toThrow(ObjectAlreadyExistsException);
    });
    ...
    it('handles archive and restore lifecycle state transitions deterministically', async () => {
      ...
      const archived = await repo.archive(sampleObject.id, '2026-08-08T14:00:00.000Z', '2026-08-08T14:00:00.000Z');
      expect(archived.status).toBe(ObjectStatus.ARCHIVED);
      expect(archived.archivedAt).toBe('2026-08-08T14:00:00.000Z');

      await expect(
        repo.archive(sampleObject.id, '2026-08-08T14:00:00.000Z', '2026-08-08T14:00:00.000Z'),
      ).rejects.toThrow(ObjectLifecycleConflictException);

      const restored = await repo.restore(sampleObject.id, '2026-08-08T15:00:00.000Z');
    ...
    it('validates pagination offset and limit inputs strictly', async () => {
      ...
      await expect(repo.list({ offset: -1 })).rejects.toThrow(ObjectValidationException);
      await expect(repo.list({ limit: -5 })).rejects.toThrow(ObjectValidationException);
    });
  });
}

describe('InMemoryObjectRepository Behavioral Contract Compliance', () => {
  const { InMemoryObjectRepository } = require('../in-memory-object-repository.js');
