...
describe('InMemoryObjectRepository Domain Contract & Micro-Hardening', () => {
  ...
  it('preserves falsy values 0, false, empty string, null, and nested arrays', async () => {
    ...
    expect(Array.isArray((created.attributes.nested as Record<string, unknown>).b)).toBe(true);
  });
  ...
  it('validates offset and limit pagination inputs deterministically', async () => {
    ...
    await expect(repo.list({ offset: 1.5 })).rejects.toThrow(ObjectValidationException);

    // 6. NaN offset rejected
    await expect(repo.list({ offset: NaN })).rejects.toThrow(ObjectValidationException);
  });
