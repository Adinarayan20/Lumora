...
export class TypographyStyles {
  public static resolveStyle(options: {
    ...
    const config =
      SemanticTypographyMap[options.role] ?? SemanticTypographyMap.Body;
    const scale = options.viewportScaleModifier ?? 1.0;

    let textColor = options.colors.textPrimary;
    if (options.colorToken === "textSecondary")
      textColor = options.colors.textSecondary;
    else if (options.colorToken === "textMuted")
      textColor = options.colors.textMuted;
    else if (options.colorToken === "primary")
      textColor = options.colors.primary;
    else if (options.colorToken === "success")
      textColor = options.colors.success;
    else if (options.colorToken === "warning")
      textColor = options.colors.warning;
    else if (options.colorToken === "danger") textColor = options.colors.danger;
    else if (options.colorToken === "inverse")
      textColor = options.colors.surface;

    let letterSpacing = 0;
    if (
      options.role === "Display XL" ||
      options.role === "Display L" ||
      options.role === "Hero"
    ) {
      letterSpacing = -0.32;
