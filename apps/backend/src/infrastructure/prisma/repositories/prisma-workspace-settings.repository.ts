...
export class PrismaWorkspaceSettingsRepository implements IWorkspaceSettingsRepository {
  ...
  public async findByWorkspaceId(workspaceId: UniqueEntityId): Promise<WorkspaceSettingsAggregate | null> {
    try {
  ...
  public toDomain(model: PrismaWorkspace): WorkspaceSettingsAggregate {
    ...
    return WorkspaceSettingsAggregate.reconstitute({
      ...
      retentionDays: RetentionDays.create((rawSettings.retentionDays as number) ?? 365),
   ...
   */
  public toPersistence(settings: WorkspaceSettingsAggregate): Record<string, unknown> {
    return {
