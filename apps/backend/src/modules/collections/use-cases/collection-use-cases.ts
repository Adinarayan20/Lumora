...
export class CreateCollectionUseCase {
  ...
  async execute(cmd: CreateCollectionCommand): Promise<Result<unknown, Error>> {
    try {
      return Result.ok(
        await this.service.createCollection(
          cmd.workspaceId,
          cmd.createdById,
          cmd.dto,
        ),
...
export class UpdateCollectionUseCase {
  constructor(private readonly service: CollectionsService) {}
  async execute(cmd: UpdateCollectionCommand): Promise<Result<unknown, Error>> {
    try {
      return Result.ok(
...
export class DeleteCollectionUseCase {
  constructor(private readonly service: CollectionsService) {}
  async execute(cmd: DeleteCollectionCommand): Promise<Result<unknown, Error>> {
    try {
      return Result.ok(
        await this.service.softDeleteCollection(
          cmd.workspaceId,
          cmd.collectionId,
          cmd.userId,
        ),
...
export class AddCollectionItemUseCase {
  ...
  async execute(
    ...
    try {
      return Result.ok(
        await this.service.addCollectionItem(
          cmd.workspaceId,
          cmd.collectionId,
          cmd.userId,
          cmd.dto,
...
export class RemoveCollectionItemUseCase {
  ...
  async execute(
    ...
    try {
      return Result.ok(
        await this.service.removeCollectionItem(
          cmd.workspaceId,
          cmd.collectionId,
          cmd.objectId,
          cmd.userId,
...
export class GetWorkspaceCollectionsQuery {
  ...
  async execute(
    ...
    try {
      return Result.ok(
        await this.service.getWorkspaceCollections(
          input.workspaceId,
          input.filter,
        ),
      );
...
export class GetCollectionQuery {
  ...
  async execute(
    ...
    try {
      return Result.ok(
        await this.service.getCollectionByIdOrSlug(
          input.workspaceId,
          input.idOrSlug,
        ),
      );
